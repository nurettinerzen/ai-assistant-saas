...
export async function executeToolLoop(params) {
  ...
  // Deterministic rescue path for pending verification.
  // Router arms state.forceToolCall only for clear verification inputs (e.g. "4567").
  // This prevents verification stalls when the model misses a tool call.
  if (state.forceToolCall?.name === 'customer_data_lookup' && state.forceToolCall?.args) {
    const forcedCall = state.forceToolCall;
    delete state.forceToolCall;

    const toolName = forcedCall.name;
    const toolArgs = forcedCall.args;
    console.log(`🧭 [ToolLoop] Executing forced tool call: ${toolName}`);
    ...
    if (repeatGuardResult.blocked) {
      console.log(`🔁 [ToolLoop] Forced call blocked by repeat guard: ${toolName} hash=${currentArgsHash}`);
      return {
    ...
    if (failPolicyResult) {
      console.error(`❌ [ToolLoop] Forced tool ${toolName} failed, returning forced response`);
      return {
    ...
    if (toolResult.data && toolName === 'customer_data_lookup') {
      state.anchor = {
    ...
    if (useStateEvents) {
      ...
      if (outcomeEvents.length > 0) {
        ...
        console.log('🧭 [ToolLoop] Applied forced-call outcome events:', outcomeEvents.map(e => e.type));
      }
    }
    ...
    if (shouldTerminate(outcome)) {
      return {
        reply: toolResult.message || GENERIC_ERROR_MESSAGES[language] || GENERIC_ERROR_MESSAGES.TR,
        inputTokens: totalInputTokens,
        outputTokens: totalOutputTokens,
        hadToolSuccess: true,
        hadToolFailure: false,
  ...
  if (state.forceToolCall) {
    console.warn(`⚠️ [ToolLoop] Unsupported forceToolCall ignored: ${state.forceToolCall.name || 'unknown'}`);
    delete state.forceToolCall;
  }
